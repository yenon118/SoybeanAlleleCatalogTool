		...
		if (gene_array.length > 0) {
			$.ajax({
				...
				success: function (response) {
					...
					if (res.length > 0) {
						let csvString = convertJsonToCsv(res);
						createAndDownloadCsvFile(csvString, String(dataset) + "_" + gene_array.join("_") + "_All_Counts");
		...
		if (gene_array.length > 0) {
			$.ajax({
				...
				success: function (response) {
					...
					if (res.length > 0) {
						let csvString = convertJsonToCsv(res);
						createAndDownloadCsvFile(csvString, String(dataset) + "_" + gene_array.join("_") + "_All_Data");
